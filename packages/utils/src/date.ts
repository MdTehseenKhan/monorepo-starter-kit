...
export function formatYearsAndMonthsSince(startDate: string, endDate: string | null): string {
...
export function formatDateMonth(inputDate: string, includeMonth = true): string {
  try {
    const parsedDate = parseISO(inputDate);
    return includeMonth ? format(parsedDate, 'MMM yyyy') : parsedDate.getFullYear().toString();
  } catch (error) {
...
export function getRelativeTimeAgo(dateString: string): string {
  ...
  for (const { unit, seconds, abbreviation } of TIME_UNITS) {
